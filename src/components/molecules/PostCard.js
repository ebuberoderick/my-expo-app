    ...
    <View className="gap-4">
      ...
      <View className="flex-row gap-4 items-center">
        <TouchableOpacity onPress={() => react()} className="flex-row items-center gap-1">
          ...
          <Text className='text-xs'>{reactedVal === "" ? parseInt(post?.likes_count) : (reactedVal === "-" ? parseInt(post?.likes_count) + (data.liked === "0" ? 1 : 0) : parseInt(post?.likes_count) - (data.liked === "1" ? 1 : 0))}</Text>
        </TouchableOpacity>
        <Animated.View>
