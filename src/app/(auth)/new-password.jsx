import { View, Text, Animated, KeyboardAvoidingView, ScrollView, Platform } from 'react-native';
        ...
        onSubmit: async (value) => {
            if (value.new_password === value.cpassword) {
                if (value.new_password.length > 7) {
                    ...
                    formHandler.setError((prevState) => ({
                        ...prevState,
                    ...
                    <View className="flex-grow justify-center">
                        <View className="absolute" style={{ top: 50, left: 12 }}>
                            <TouchableOpacity
                                ...
                                className="items-center justify-center border border-gray-300 rounded-full"
                            >
                                <FontAwesome
                                    name="angle-left"
                                    size={30}
                        ...
                        <View>
                            <Animated.Image
                                source={require("../../assets/images/successkey.png")}
                                style={{ width: "90%" }}
                                className="mx-auto relative h-auto"
                            />
                        </View>
                    </View>
                    <View className="gap-7 px-3">
                        <Text className="text-2xl font-extrabold">Please enter your new password</Text>
                        <Text>Don’t worry! Please enter your registered email address below for reset code.</Text>
                        <View className="gap-5 pb-9">
                            <AppInput
                                error={formHandler.error?.new_password}
                                onChange={e => (formHandler.value.new_password = e)}
                                icon={<Ionicons name="lock-open-outline" size={25} color={"#9ca3af"} />}
                                placeholder={"Enter New Password"}
                                type={"password"}
                            />
                            <AppInput
                                error={formHandler.error?.cpassword}
                                onChange={e => (formHandler.value.cpassword = e)}
                                icon={<Ionicons name="lock-open-outline" size={25} color={"#9ca3af"} />}
